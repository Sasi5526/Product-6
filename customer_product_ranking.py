...
sales_df = pd.read_csv('C:\\Users\\sasim\\OneDrive\\Desktop\\product\\cleaned_cluster.csv')

#Items a Customer purchased the most
# Find the no of units sold of each product by customer
top_sell_cust_items_df = sales_df.groupby(['user_id','product_category']).agg({'review_score':'sum'})
...
qty_col = top_sell_cust_items_df['review_score'].astype(str)
...
unique_order_items_df = sales_df.drop_duplicates(['user_id','product_category','order_purchase_timestamp'])


# Find the no of orders placed and the unique no of customers placed orders, of each product
freq_items_df = unique_order_items_df.groupby(['user_id','product_category']).agg({'order_purchase_timestamp':'count'})
...
cust_prod_rankings_df = pd.merge(top_sell_cust_items_df,freq_items_df,how='inner',on=['user_id','product_category'])
...
items_price_df = sales_df.groupby(['product_category']).agg({'price':'max'})
...
cust_prod_rankings_df = pd.merge(cust_prod_rankings_df,items_price_df,how='left',on='product_category')


# Get only the Customer, Product, Price and Rank columns
cust_prod_rankings_df = cust_prod_rankings_df[['user_id','product_category','price','review_score','Top_Sell_Rank','No_of_Orders','Popularity_Rank']]
...
cust_prod_rankings_df.to_csv('C:\\Users\\sasim\\OneDrive\\Desktop\\product\\Customer-Product-Rankings.csv',index=False)
