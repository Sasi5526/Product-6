...
top_sell_items_df = sales_df.groupby('product_category').agg({'review_score':'sum', 'price':'max'})
...
top_sell_items_df['Top_Sell_Rank'] = top_sell_items_df['review_score'].rank(method='min',ascending=False).astype(int)
...
unique_order_items_df = sales_df.drop_duplicates(['product_category','order_purchase_timestamp','user_id'])


# Find the no of orders placed and the unique no of customers placed orders, of each product
most_popular_items_df = unique_order_items_df.groupby('product_category').agg({'order_purchase_timestamp':'count', 'user_id':'nunique'})
...
product_rankings_df = pd.merge(top_sell_items_df,most_popular_items_df,how='inner',on='product_category')

# Get only the Product, Price and Rank columns
product_rankings_df = product_rankings_df[['product_category','price','Top_Sell_Rank','Popularity_Rank']]

# List the Product Rankings
#product_rankings_df.sort_values('Popular

product_rankings_df.to_csv('C:\\Users\\sasim\\OneDrive\\Desktop\\product\\Product-Rankings.csv',index=False)
