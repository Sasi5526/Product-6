...
def similar_prods(prod_name):
    ...
    similar_prods = pd.merge(similar_prods_corr,prod_ranking_model[['product_category','price']],how='left',on='product_category')
    
    prod_price = similar_prods[similar_prods['product_category'] == prod_name]['price'].values[0]
    
    input_prod_index = similar_prods[similar_prods['product_category'] == prod_name].index
