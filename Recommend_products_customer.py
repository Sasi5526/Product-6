...
prod_cust_qty_df = sales_df.groupby(['product_category','user_id']).agg({'review_score':'sum'})
...
prod_cust_count_df = sales_df.groupby(['product_category']).agg({'user_id':'nunique'})
...
prod_cust_df = pd.merge(prod_cust_qty_df,prod_cust_count_df,how='inner',on='product_category')


# Create a pivot table with all Customers on columns and Products on rows, and Qty as values
prod_cust_pivot_df = prod_cust_df.pivot(index='product_category',columns='user_id',values='review_score').fillna(0)
...
cust_correlation_df.to_csv('C:\\Users\\sasim\\OneDrive\\Desktop\\product\\Customer-Customer-Correlation-Matrix.csv')
